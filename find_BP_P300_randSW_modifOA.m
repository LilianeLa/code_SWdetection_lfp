...
% 	   			 >> 			 [countBP, countP300, countrandSW] = find_BP_P300_randSW(ttl.times, SW, SA34_20_06_2014_0003.values)		% !! Don't forget 'values' otherwise length(V)=1
% 
% 		- Or open the corresponding 'SW.mat' if already saved.
% 	   			 >> 			 [countBP, countP300, countrandSW] = find_BP_P300_randSW_modifOA(ttl.times, SW, SA34_20_06_2014_0003.values)		% !! Don't forget 'values' otherwise length(V)=1
...
function [countBP, countP300, countrandSW] = find_BP_P300_randSW_modifOA(vertical, SW, V)				% The input is in fact ttl.times but we can't write "function find_BP(ttl.times)" although we can write "find_BP(ttl.times)"	in the prompt
...
for i = 1 : length(oa_new)
	...
	if randwindow(1) == 0		% Adding 31 May 
		randwindow(1) =1;
	end 
...
elseif countrandSW == 1 	
	disp([' - 1 of the ', num2str(countOA), ' random windows shows at least one SW (which is neither BP nor P3).'])
else 
